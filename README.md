...
│  ├─ weather-macro.js                 # Код макроса генератора морской погоды
│  ├─ calculator/
...
3. `weather-macro.js` -> код макроса погоды;
...
- `ready`:
  ...
  - создаёт/обновляет макрос `Zephyr's Sea Travel Calculator` (только для GM);
  - создаёт/обновляет макрос `Генератор морской погоды` (только для GM).
...
- компас с выбором направления ветра и курса движения;
...
- `uiLanguage` (`scope: client`, `config: true`) — язык интерфейса калькулятора.
...
## 13. Локализация и язык интерфейса
...
- Чтобы добавить новый язык, нужно:
  1. Дополнить `ZEPHYR_I18N` новыми ключами;
  2. Добавить `*_xx` поля в данных кораблей и условий;
  3. Обновить тексты в UI/чате по необходимости.

## 14. Технические нюансы и ограничения
...
- Сохранение `windowSettings` также дебаунсится и не пишет при отсутствии изменений.

## 15. Быстрая проверка после изменений
...
2. Проверить пересчёт при смене:
   ...
   - включения/выключения вёсел (для кораблей с вёслами);
...
5. В настройках модуля переключить язык и убедиться, что UI/чат меняют язык.
