class WeatherGeneratorUI {
  ...
  async generateWeather(html, ctx) {
    try {
      const { moduleId, flags, climate, zoneKeys, lang } = ctx;
      const zone = html.find("#zone").val();
      const season = html.find("#season").val();
      const currentHour = parseInt(html.find("#currentHour").val(), 10) || 12;
      const tod = WeatherGeneratorUI.hourToCategory(currentHour);
      const gmOnly = !!html.find("#gmOnly")[0]?.checked;

      await Promise.all([
        game.user.setFlag(moduleId, flags.F_ZONE, zone),
        game.user.setFlag(moduleId, flags.F_SEASON, season)
      ]);
      ...
      await game.user.setFlag(moduleId, flags.F_HOUR, nextHour);
      ...
      this.saveLastInput({ zone, season, currentHour: nextHour, gmOnly });
    } catch (err) {
